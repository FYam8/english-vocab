...
def fourth_validation_green(data: dict) -> bool:
    v = data.get("fourthPromotionValidation", {})
    return (
        data.get("fourthPromotionStatus") == "promoted"
...
def validate_promoted(data: dict, engine: str, core: str) -> None:
    if data.get("fifthPromotion") != SYMBOL or data.get("fifthPromotionStatus") != "promoted":
        raise SystemExit("fifth promotion contract is not in promoted state")
    if BLOCK not in engine:
        raise SystemExit("fifth promoted helper missing from engine candidate")
    if BLOCK in core:
        raise SystemExit("fifth promoted helper still duplicated in v7.6 core")
    promoted = [x for x in data.get("promotedSchoolNeutralHelpers", []) if x.get("symbol") == SYMBOL]
    if len(promoted) != 1:
        raise SystemExit("fifth promoted helper contract entry is missing or duplicated")
    item = promoted[0]
    if item.get("previousSource") != "35-v76-memory-runtime.js" or item.get("target") != "20-engine-candidate.js":
        raise SystemExit("fifth promoted helper ownership metadata changed unexpectedly")
...
def main() -> None:
    ...
    if not fourth_validation_green(data):
        raise SystemExit("refusing fifth promotion until the fourth helper has exact-head and synthetic-merge green validation")

    if data.get("fifthPromotionStatus") == "promoted":
        validate_promoted(data, engine, core)
        print("Fifth common-engine helper promotion: ALREADY APPLIED")
        return

    if data.get("fifthPromotion") != SYMBOL or data.get("fifthPromotionStatus") != "pending":
        raise SystemExit("unexpected fifth-promotion contract state")

    ready = [x for x in data.get("readySchoolNeutralHelpers", []) if x.get("symbol") == SYMBOL]
    if len(ready) != 1:
        raise SystemExit("fifth promotion must have exactly one ready-helper entry")
    item = ready[0]
    if item.get("source") != "35-v76-memory-runtime.js" or item.get("target") != "20-engine-candidate.js":
        raise SystemExit("fifth promotion source/target changed without review")

    if core.count(BLOCK) != 1:
        raise SystemExit("reviewed v76LapseStability block is not unique")
    if BLOCK in engine:
        raise SystemExit("v76LapseStability already exists in engine candidate while contract says pending")
    if engine.count(TARGET_ANCHOR) != 1:
        raise SystemExit("engine insertion anchor for v76LapseStability is not unique")

    for token in data.get("blockedTokensForReadyHelpers", []):
        if token in BLOCK:
            raise SystemExit(f"refusing v76LapseStability promotion with blocked Waseda token: {token}")
    if any(x in BLOCK for x in ["getProgress(", "schedulerScore(", "VOCAB", "state.", "session.", "priority", "studyLayer"]):
        raise SystemExit("refusing policy-coupled v76LapseStability promotion")
    ...
    if core_after.count(BLOCK) != 0 or engine_after.count(BLOCK) != 1:
        raise SystemExit("v76LapseStability move did not produce a single canonical definition")
    ...
    data["fifthPromotionStatus"] = "promoted"
    ...
    print("Fifth common-engine helper promotion: PASS")
