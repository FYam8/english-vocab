...
function evalLiteral(marker){return Function(`"use strict"; return (${extractInitializer(html,marker)});`)();}
...
function flags(ex){
  const en=enText(ex), ja=String(ex?.ja||'').trim(),out=[];
...
const pairs=[],yearCoverage=[],suspicious=[];
for(const v of VOCAB){
  const ex=EXAM_EXAMPLES[v.id]||null;
  const row={id:v.id,word:v.word,meaning:v.meaning,years:v.years||[],layer:v.studyLayer||'core',level:v.level,example:ex?{year:ex.year,source:ex.source,page:ex.page,mode:ex.mode,sentence:ex.sentence,fragment:ex.fragment,matchedForm:ex.matchedForm,ja:ex.ja,note:ex.note}:null};
  pairs.push(row);
  const fsx=flags(ex);if(fsx.length)suspicious.push({...row,flags:fsx});
  for(const y of(v.years||[]))yearCoverage.push({id:v.id,word:v.word,year:y,representativeYear:ex?.year??null,sameYear:Number(ex?.year)===Number(y),hasRepresentative:!!ex});
}
const summary={vocabCount:VOCAB.length,exampleCount:Object.keys(EXAM_EXAMPLES).length,pairCount:pairs.length,yearPairs:yearCoverage.length,sameYearCount:yearCoverage.filter(x=>x.sameYear).length,fallbackCount:yearCoverage.filter(x=>x.hasRepresentative&&!x.sameYear).length,missingExampleCount:yearCoverage.filter(x=>!x.hasRepresentative).length,suspiciousCount:suspicious.length,orphanExamples:Object.keys(EXAM_EXAMPLES).filter(id=>!byId.has(id)).length};
...
for(let i=0;i<pairs.length;i+=80){
  const idx=String(i/80+1).padStart(2,'0'),batch=pairs.slice(i,i+80);
  fs.writeFileSync(`qa/content-audit/pairs-${idx}.json`,JSON.stringify(batch,null,2));
  fs.writeFileSync(`qa/content-audit/compact-${idx}.tsv`,batch.map(r=>[r.id,r.word,r.example?.year??'',clean(r.example?.sentence||r.example?.fragment||r.example?.matchedForm),clean(r.example?.ja)].join('\t')).join('\n')+'\n');
}
const reportedWords=new Set(['believe','lot','close','base']);
fs.writeFileSync('qa/content-audit/reported-cases.json',JSON.stringify(pairs.filter(r=>reportedWords.has(r.word)),null,2));
