...
for (const key of ['branchTwoPass','syntheticMergeTwoPass','branchOwnership','syntheticMergeOwnership','assemble']) {
  assert.equal(secondApproval[key], 'success', `challenge-score boundary approval incomplete: ${key}`);
}
...
for (const key of ['branchTwoPass','syntheticMergeTwoPass','branchOwnership','syntheticMergeOwnership','assemble']) {
  assert.equal(secondRuntime[key], 'success', `challenge-score runtime validation incomplete: ${key}`);
}
...
assert.equal(validation.allowThirdPlanningAdapterGroupIntroduction, false);
assert.equal(validation.thirdProposedGroup, 'challenge-session-composition-policy-adapter');
assert.equal(validation.thirdContractPath, 'src/common-engine/challenge-session-composition-contract.json');
assert.equal(validation.thirdContractStatus, 'contract-definition-only-awaiting-green-validation');
...
]) assert.ok(validation.thirdProposedGroupMustPreserve.includes(required), `missing third-group preservation rule: ${required}`);
...
assert.deepEqual(thirdContract.exactBehavior.foundationFilterEvaluationOrder, [
  ...
  'year eligibility',
  'getProgress(v.id)',
  'derive due and recent-mistake flags',
  'weak OR due OR recent-mistake state eligibility'
]);
...
assert.deepEqual(thirdContract.adapterDesign.allowedMethods, [
  ...
  'requiredChallengeCount(desired)',
  'foundationExceptionCap(desired)'
]);
...
for (const forbidden of [
  ...
  'changing challenge session composition before the third contract is green',
...
]) assert.ok(validation.forbidden.includes(forbidden), `missing third-contract prohibition: ${forbidden}`);
